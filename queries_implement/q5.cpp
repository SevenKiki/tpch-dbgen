...
void Project_Option(Project_Node &project_node)
{
  ...
  resfile.open("test.log",std::ios::out | std::ios::trunc);
  for (int i = 0; i < project_node.colnum; i++)
  {
...
int main()
{
  if (access(orders_arrow_dir, F_OK) != 0)
  {
    ...
    Load_From_CSV_To_IPC();
  }
  Load_From_IPC_To_ARROW(lineitem_t, partsupp_t, orders_t, part_t, supplier_t, customer_t, nation_t, region_t);
  ...
  *(group_node.group_data[0].group_count) = 0;
  ...
  Select_Option(select_node[0], 72);
  /*select option on nation*/
  Select_Option(select_node[1], 72);
  /*group option on nation*/
  Group_Option(group_node, 72);
  /*select option on supplier*/
  Select_Option(select_node[2], 72);
  /*select option on customer*/
  Select_Option(select_node[3], 72);
  /*select option on orders*/
  Select_Option(select_node[4], 72);
  ...
  std::cout << *(group_node.group_total_num) << std::endl;
  for (int i = 0; i < 72; i++)
  {
  ...
  /*int nums = lineitem_t->num_rows()/72;
  for (int i = 1; i < 72; i++)
  {
  ...
  for (int i = 0; i < join_node.index[0]; i++)
    std::cout << join_node.OID[i] << " ";
    
  std::cout << join_node.index[0] <<std::endl;
  project_node.group_total_num = join_node.index[0];
  project_node.project_data[0].res_array = new int[join_node.index[0]];
  project_node.project_data[0].name_array = "o_orderdate";
  project_node.project[0] = project_int;
  project_node.write[0] = write_int;
  project_node.colnum = 3;
  project_node.project_data[0].pro_sel = Load_int_column_from_table("orders", "o_orderdate");
  project_node.project_data[0].FK_sel[0] = Load_int_column_from_table("lineitem", "l_orderkey_1");
  project_node.project_data[0].OID = join_node.OID;
  project_node.project_data[0].fk_num = 1;
  project_node.project_data[0].group_count = join_node.index[0];

  project_node.project_data[1].res_array = new int[join_node.index[0]];
  project_node.project_data[1].name_array = "l_orderkey_1";
  project_node.project[1] = project_int;
  project_node.write[1] = write_int;
  project_node.project_data[1].pro_sel = Load_int_column_from_table("lineitem", "l_orderkey_1");
  project_node.project_data[1].OID = join_node.OID;
  project_node.project_data[1].fk_num = 0;
  project_node.project_data[1].group_count = join_node.index[0];

  project_node.project_data[2].res_array = new int[join_node.index[0]];
  project_node.project_data[2].name_array = "n_name";
  project_node.project[2] = project_int;
  project_node.write[2] = write_int;
  project_node.project_data[2].pro_sel = Load_int_column_from_table("nation", "n_regionkey");
  project_node.project_data[2].FK_sel[0] = Load_int_column_from_table("lineitem", "l_suppkey");
  project_node.project_data[2].FK_sel[1] = Load_int_column_from_table("supplier", "s_nationkey");
  project_node.project_data[2].OID = join_node.OID;
  project_node.project_data[2].fk_num = 2;
  project_node.project_data[2].group_count = join_node.index[0];*/
